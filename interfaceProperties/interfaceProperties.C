...
Foam::interfaceProperties::maranForce() const
{
    volVectorField gradSigma = fvc::grad(sigmaPtr_->sigma());

    volVectorField gradAlpha = fvc::grad(alpha1_);

    volVectorField maranForce = gradSigma - gradAlpha * ( gradAlpha & gradSigma);
